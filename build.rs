...
fn main() {
    #[cfg(feature = "cuda")]
    {
        ...
        if !std::path::Path::new(&lib_file).exists() && !std::path::Path::new(&lib_file_v2).exists() {
            println!("cargo:warning=libcutensor.so not found at {}. Linking may fail.", lib_path);
        ...
        } else if std::path::Path::new(&lib_file_v2).exists() && !std::path::Path::new(&lib_file).exists() {
            println!("cargo:warning=Found libcutensor.so.2 but not libcutensor.so - you may need to create a symlink:");
            println!("cargo:warning=  ln -s libcutensor.so.2 {}/libcutensor.so", lib_path);
        }
    }
}
