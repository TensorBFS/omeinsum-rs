...
fn main() {
    for (m, k, n) in [(32, 32, 32), (64, 64, 64), (64, 64, 64), (256, 128, 256), (1024, 256, 1024), (2048, 512, 2048)] {
        ...
        println!("GEMM {}x{}x{}: {:.2} µs ({:.1} GFLOPS)", m, k, n, avg_us, gflops);
    }
}
...
fn faer_gemm(a: &[f32], m: usize, k: usize, b: &[f32], n: usize, c: &mut [f32]) {
    ...
    let flops = 2 * m * k * n;
    let par = if flops < GEMM_PARALLEL_THRESHOLD { Par::Seq } else { Par::rayon(0) };
    ...
    if !PRINTED.swap(true, std::sync::atomic::Ordering::Relaxed) {
        println!("Threshold: {} FLOPs, using {:?} for {}x{}x{} ({} FLOPs)",
                 GEMM_PARALLEL_THRESHOLD,
                 if flops < GEMM_PARALLEL_THRESHOLD { "Seq" } else { "Parallel" },
                 m, k, n, flops);
    }
}
